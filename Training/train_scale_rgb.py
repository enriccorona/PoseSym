...
if __name__ == "__main__":
  ...
  for epoch in range(1, epochs + 1):
      train(epoch,np.array(X_train),Y_train,labels_train,X_views,Y_multiview,labels_views)
      if(epoch%1 == 0):
        acc = test(epoch,X_test,Y_test,labels_test,X_views,Y_multiview,labels_views)
        if acc > best_acc:
          best_acc = acc
  
          if considering_symmetries:
          torch.save(model_S, 'model_train_pretrained_symms_S_' + str(numb_views))
          torch.save(model_V, 'model_train_pretrained_symms_S_' + str(numb_views))
          else:
          torch.save(model_S, 'model_train_pretrained_nosymms_S_' + str(numb_views))
          torch.save(model_V, 'model_train_pretrained_nosymms_S_' + str(numb_views))
